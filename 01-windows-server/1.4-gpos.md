...
For these reasons, this organization wants to instill a policy stating: **Domain workstations do not permit removable-storage access by default**. We will now help them implement this.

#### Configuration 

To configure this policy, we create our GPO under the `Workstations` OU and name it `REMOVABLE-STORAGE-RESTRICTION`. 

PIC

We then navigate within this GPO to 'Removable Storage Access' under Computer Configuration. For the 'All Removable Storage classes' we deny all access.

PIC

#### Verification

update client with gpupdate /force and view result with gpresult /r. check against 2 user accounts on client vm. 

PIC

take ss of before gpo is implented on client of having access to removable storage

PIC

take ss of after 

Link to client overview where I mount a vmdk file as a USB and my general process for doing so, disclaimer that this is not an officially released method 
