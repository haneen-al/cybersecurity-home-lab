...
The next step is very important. We only want this configuration applied to our single IT Administrator account. Using **Security Filtering**, we can target only the `IT-ADMINS` security group. 

Under the scope of `IT-ADMIN-POLICY`, we navigate to **Security Filtering**, and replace 'Authenticated Users' with 'IT-ADMINS'.
...
Aftering running an Admin CMD on the Windows client, running `gpupdate /force`, and restarting the computer, we should be able to log into Haneen's Admin account to verify configurations. 

We start by using the command `whoami /groups` and `net localgroup Administrators`. Both commands reflect that this account now works as a local admin account. 
...
As opposed to a standard user account where when we try to access an elevated CMD, we are met with this: 
